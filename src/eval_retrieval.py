...
def load_qrels(path: Path) -> dict:
    ...
    for row in read_jsonl(path):
        ...
        did = str(row.get("corpus-id") or row.get("doc_id") or row.get("corpus_id") or "")
...
def main():
    ...
    print("=" * 56)
    for k in K_VALUES:
        print(f"  NDCG@{k:<17} {mean(faiss_ndcg[k]):>12.4f} {mean(rerank_ndcg[k]):>14.4f}")
    print("-" * 56)
    for k in K_VALUES:
        print(f"  Recall@{k:<15} {mean(faiss_recall[k]):>12.4f} {mean(rerank_recall[k]):>14.4f}")
    print("-" * 56)
    print(f"  {'MRR':<22} {fm:>12.4f} {rm:>14.4f}")
    print("=" * 56)
    print(f"\n  Latency (avg per query, end-to-end)")
